...
struct acceptor *acceptor_init(int port)
{
	...
	int ret1 = bind(acceptor1->listen_fd, (struct sockaddr *)&server_addr, sizeof(server_addr));
	if(ret1 < 0)
	{
		error(1,errno, "bind failed");
	}
	
	int ret2 = listen(acceptor1->listen_fd, LISTENQ);
	if(ret < 0)
	{
		error(1, error, "listen failed");
	}
