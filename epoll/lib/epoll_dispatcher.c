...
static int epoll_dispatcher(struct event_loop *, struct timeval *);
...
void *epoll_init(struct event_loop *eventLoop)
{
    ...
    epollDispatcherData->efd = epoll_create(0);
...
int epoll_dispatch(struct event_loop *eventLoop, struct timeval *timeval)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if(epollDispatcherData->events[i].events & EPOLLIN)
        {
            ...
            channel_event_active(eventLoop,epollDispatcherData->events[i].data.fd, EVENT_READ);
        }
        if(epollDispatcherData->events[i].events & EPOLLOUT)
        {
            yolanda_msgx("get message channel fd=%d for read, %s", epollDispatcherData->eventsp[i].data.fd, eventLoop->thread_name);
            channel_event_active(eventLoop, epollDispatcherData->events[i].data.fd, EVENT_WRITE);
        }
    }
    return 0;
}
...
{
    epoll_dispatcher_data *epollDispatcherData = (epollDispatcherData *)eventLoop->event_dispatcher_data;
